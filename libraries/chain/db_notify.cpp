...
void operation_get_impacted_accounts( const operation& op, flat_set<account_id_type>& result, database *db = nullptr )
{
  get_impacted_account_visitor vtor = get_impacted_account_visitor( result, db );
  op.visit( vtor );
}

void transaction_get_impacted_accounts( const transaction& tx, flat_set<account_id_type>& result, database *db = nullptr )
{
  for( const auto& op : tx.operations )
    operation_get_impacted_accounts( op, result, db );
}

void get_relevant_accounts( const object* obj, flat_set<account_id_type>& accounts, database *db = nullptr )
{
      ...
      switch( (object_type)obj->id.type() )
      {
        ...
        case account_object_type:{
          ...
          break;
           ...
           assert( aobj != nullptr );
           transaction_get_impacted_accounts( aobj->proposed_transaction, accounts, db );
           break;
        } case operation_history_object_type:{
           const auto& aobj = dynamic_cast<const operation_history_object*>(obj);
           assert( aobj != nullptr );
           operation_get_impacted_accounts( aobj->op, accounts, db );
           break;
           ...
           break;
            ...
            const auto& aobj = dynamic_cast<const construction_capital_vote_object*>(obj);
            if (db) {
                {
                    auto& index = db->get_index_type<construction_capital_vote_index>().indices().get<by_vote_from>();
                    auto it = index.find(aobj->cc_from);
                    if (it != index.end()) {
                        auto& index_cc = db->get_index_type<construction_capital_index>().indices().get<by_id>();
                        auto it_cc = index_cc.find(it->cc_from);
                        if (it_cc != index_cc.end()) {
                            accounts.insert(it_cc->owner);
                        }
                    }
                }
                {
                    auto& index = db->get_index_type<construction_capital_vote_index>().indices().get<by_vote_to>();
                    auto it = index.find(aobj->cc_to);
                    if (it != index.end()) {
                        auto& index_cc = db->get_index_type<construction_capital_index>().indices().get<by_id>();
                        auto it_cc = index_cc.find(it->cc_to);
                        if (it_cc != index_cc.end()) {
           ...
           const auto& aobj = dynamic_cast<const limit_order_fee_config_object*>(obj);
           accounts.insert( aobj->receiver );
           break;            
        } case construction_capital_rate_vote_object_type:{
            const auto& aobj = dynamic_cast<const construction_capital_rate_vote_object*>(obj);
   ...
   else if( obj->id.space() == implementation_ids )
   {
      switch( (impl_object_type)obj->id.type() )
      {
             ...
             case impl_account_balance_object_type:{
              ...
              assert( aobj != nullptr );
              transaction_get_impacted_accounts( aobj->trx, accounts, db );
              break;
